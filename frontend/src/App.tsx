...
import {
  Activity, AlertTriangle, CheckCircle, Cpu, FileText, Heart,
  LayoutDashboard, Play, Settings as SettingsIcon, Users,
  type LucideIcon,
} from 'lucide-react';
import {
  Claims, Creators, Dashboard, Notes, Processing, Review, Settings,
} from './pages/AllPages';

type PageId = 'dashboard' | 'creators' | 'processing' | 'notes' | 'review' | 'claims' | 'settings';
...
const PAGE_TITLES: Record<PageId, string> = {
  dashboard: 'Dashboard', creators: '创作号管理', processing: '处理队列',
  notes: '笔记浏览', review: '笔记审查', claims: '待验证声明', settings: '全局设置',
};
...
export default function App() {
  ...
  useEffect(() => {
    let alive = true;
    const check = async () => {
      try { const r = await fetch('/api/health'); if (alive) setHealthy(r.ok); }
      ...
      <div className="flex-1 flex flex-col ml-60 min-w-0">
        <header className="h-14 border-b border-slate-800 bg-slate-900/50 backdrop-blur flex items-center justify-between px-6 sticky top-0 z-10">
          ...
          <div className="flex items-center gap-2 text-sm">
            {healthy === null ? (
              <span className="badge badge-gray flex items-center gap-1"><Heart size={12} /> 检查中</span>
            ) : healthy ? (
              <span className="badge badge-green flex items-center gap-1"><Heart size={12} /> 在线</span>
            ) : (
              <span className="badge badge-red flex items-center gap-1"><Heart size={12} /> 离线</span>
            )}
          </div>
        </header>
